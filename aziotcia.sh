...
if [ $IS_IOTHUB_DEPLOY_STR == "true" ];then
  ...
  az iot dps enrollment-group create -g $RESOURCE_GROUP_NAME --dps-name  $DPS_NAME --enrollment-id 'arubaenrollmentid' --primary-key $DPS_ENROLMENT_PRIMARY_KEY  --secondary-key $DPS_ENROLMENT_SECONDARY_KEY
  echo "End iot Hub"
else
